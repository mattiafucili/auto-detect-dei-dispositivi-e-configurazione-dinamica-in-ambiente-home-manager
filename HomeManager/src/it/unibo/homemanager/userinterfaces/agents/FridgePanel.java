...
public class FridgePanel extends javax.swing.JPanel {
	private static final long serialVersionUID = 1L;

	private FridgeAgent fridgeAgent;
	private ViewManageAgentPanel butlerPanel;

	public FridgePanel(ViewManageAgentPanel butlerPanel, FridgeAgent fridgeAgent, TracePanel tracePanel) {
		this.fridgeAgent = fridgeAgent;
		this.butlerPanel = butlerPanel;
		
		initComponents();
		
		getFridgeAgent().addStateListener(new StateChangeListener() {
			@Override
		...
		getFridgeAgent().addIngredientsListener(new IngredientsChangeListener() {	
			@Override
	...
	public FridgeAgent getFridgeAgent() {
		return (fridgeAgent);
	}
	...
	private void onClickButtonAdd(ActionEvent event) {
		...
		if (choice == JOptionPane.OK_OPTION) {
			try {
				fridgeAgent.addIngredient(new Ingredient(nameProduct, quantity));
				this.quantityProductText.setText("");
				this.productNameText.setText("");
				getFridgeAgent().getContent();
				return;
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	private void onClickButtonDelete(ActionEvent event) {
		...
		if (choice == JOptionPane.OK_OPTION) {
			try {
				fridgeAgent.updateContent(new Ingredient(element_selected.getName(), quantity_total));
	...
	private void onStateChangedEvent() {
		addToListButton.setEnabled(getFridgeAgent().getState());
		jButton2.setEnabled(getFridgeAgent().getState());
	}
	
	private void onIngredientsChangedEvent() {
		listModel1.clear();
		List<Ingredient> ingredients = getFridgeAgent().getCurrentIngredients();
